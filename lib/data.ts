export const personalInfo = {
  name: "Rishav Roy",
  initials: "RR",
  tagline: "Aspiring Developer & AI/ML Enthusiast",
  bio: "I am a second-year B.Tech student specializing in Computer Science with AI & ML at Narula Institute of Technology. Passionate about technology, problem-solving, and continuous learning.",
  email: "rishavroy2006@gmail.com",
  location: "Kolkata, India",
};

export const socialLinks = {
  github: "https://github.com/Rishavroy-2006",
  linkedin: "https://www.linkedin.com/in/rishav-roy2006",
  instagram: "https://instagram.com/rishav.royy",
};

export const skills = [
  { name: "C", category: "language", level: 85 },
  { name: "C++", category: "language", level: 80 },
  { name: "Java", category: "language", level: 75 },
  { name: "Python", category: "language", level: 85 },
  { name: "HTML", category: "web", level: 90 },
  { name: "CSS", category: "web", level: 85 },
  { name: "JavaScript", category: "web", level: 80 },
  { name: "Git", category: "tool", level: 75 },
  { name: "VS Code", category: "tool", level: 90 },
  { name: "AI/ML", category: "specialty", level: 70 },
];

export const projects = [
  {
    title: "Swapnacharon",
    description: "Photography Portfolio website featuring cinematic layouts and visual galleries along with social media highlights.",
    tech: ["HTML", "CSS", "JavaScript"],
    link: "https://rishavroy-2006.github.io/Swapnacharon/",
    github: "https://github.com/RishavRoy-2006/Swapnacharon",
  },
  {
    title: "AI-ML Timetable Manager",
    description: "A responsive timetable interface designed to help students organize class schedules quickly.",
    tech: ["React", "Next.js", "Tailwind CSS"],
    link: "https://aiml2a-time-table.vercel.app/",
    github: "https://github.com/Rishavroy-2006",
  },
  {
    title: "Raj Mahal Review Portal",
    description: "A Banquet review platform where users can explore ratings and share their experiences.",
    tech: ["React", "Next.js", "Vercel"],
    link: "https://v0-raj-mahaal-review-portal-755qq9zl8.vercel.app/",
    github: "https://github.com/Rishavroy-2006",
  },
  {
    title: "UIDIA Aadhaar Data Console",
    description: "A conceptual government dashboard interface for visualizing identity data and analytics.",
    tech: ["React", "Next.js", "Data Visualization"],
    link: "https://uidia-aadhar-data-console.vercel.app/",
    github: "https://github.com/Rishavroy-2006",
  },
  {
    title: "PhoneCloud",
    description: "An experimental web interface simulating a cloud environment for accessing phone data and files.",
    tech: ["Web Technologies", "Cloud Simulation"],
    link: "https://phonecloud.serveousercontent.com/",
    github: "https://github.com/Rishavroy-2006",
  },
];

export const education = [
  {
    degree: "Secondary Education",
    specialization: "High School",
    institution: "Adamas International School",
    period: "Apr 2009 - Apr 2024",
    status: "Completed",
  },
  {
    degree: "B.Tech in Computer Science & Engineering",
    specialization: "Artificial Intelligence & Machine Learning",
    institution: "Narula Institute of Technology - West Bengal, India",
    period: "2024 - 2028 (Expected)",
    status: "2nd Year Student",
    cgpa: "8.2/10",
    coursework: [
      "Data Structures and Algorithms",
      "Object-Oriented Programming",
      "Database Management Systems",
      "Machine Learning Fundamentals",
      "Web Technologies",
      "Software Engineering",
    ],
  },
];

export const currentlyLearning = [
  { name: "Data Structures & Algorithms", progress: 65 },
  { name: "Machine Learning Fundamentals", progress: 45 },
  { name: "React & Next.js", progress: 55 },
  { name: "System Design Basics", progress: 30 },
];

export const beyondCode = [
  "Gaming",
  "Experimenting with new technologies",
  "Building weird web experiments",
  "Exploring creative UI ideas",
];

export const navItems = [
  { name: "Home", href: "#home" },
  { name: "About", href: "#about" },
  { name: "Skills", href: "#skills" },
  { name: "Projects", href: "#projects" },
  { name: "Education", href: "#education" },
  { name: "Contact", href: "#contact" },
];
